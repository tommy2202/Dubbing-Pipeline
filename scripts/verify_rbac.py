...
def main() -> int:
    with tempfile.TemporaryDirectory() as td:
        ...
        with TestClient(app) as c_view:
            ...
            assert (
                c_view.post("/api/uploads/init", json={"filename": "x.mp4", "total_bytes": 1}).status_code
                == 403
            )
            assert (
                c_view.put(
        ...
        with TestClient(app) as c_ed:
            ...
            assert c_ed.post("/api/uploads/init", json={"filename": "x.mp4", "total_bytes": 1}).status_code == 403
            assert (
                c_ed.put(
                    "/api/jobs/j_rbac_1/overrides",
                    json={"speaker_overrides": {"1": "SPEAKER_01"}},
