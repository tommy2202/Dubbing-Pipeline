...
def main() -> int:
    with tempfile.TemporaryDirectory() as td:
        ...
        (in_root / "Test.mp4").write_bytes(b"\x00" * 1024)
        ...
        with TestClient(app) as c:
            c.app.state.job_store.put(
                Job(
                    id="j_keys_1",
                    owner_id="u_ed_k",
                    video_path=str(in_root / "Test.mp4"),
                    duration_s=1.0,
                    mode="low",
                    device="cpu",
                    src_lang="ja",
        ...
        with TestClient(app) as c_admin:
            ...
            k_view = c_admin.post(
                "/keys", json={"user_id": "u_view_k", "scopes": ["read:job"]}, headers=hdr
            ).json()["key"]
            k_submit = c_admin.post(
                "/keys", json={"user_id": "u_op_k", "scopes": ["submit:job"]}, headers=hdr
            ).json()["key"]
            k_edit = c_admin.post(
                "/keys", json={"user_id": "u_ed_k", "scopes": ["edit:job"]}, headers=hdr
            ).json()["key"]
