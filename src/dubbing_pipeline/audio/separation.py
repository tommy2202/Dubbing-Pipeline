...
from dubbing_pipeline.utils.ffmpeg import ensure_wav_44k_stereo
...
def separate_dialogue(
    ...
    *,
    ...
    logger.info("[dp] separation: demucs start", model=str(model), device=str(device), key=key)
    ...
    logger.info("[dp] separation: demucs ok", dialogue=str(dlg), background=str(bg))
    return SeparationResult(
