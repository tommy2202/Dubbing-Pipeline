    ...
    *,
    ...
    if job_id and out_mkv.exists():
        ...
        if stage_is_done(ckpt, "mux"):
            logger.info("[dp] mux stage checkpoint hit")
            return out_mkv

    vid_dur = _ffprobe_duration_s(src_video)
    if vid_dur is None:
        logger.warning("[dp] ffprobe duration unavailable; mux will be best-effort")

    # If SRT is missing/empty, don't attempt to mux subtitles (ffmpeg fails on empty file).
    if srt_path is not None:
        try:
            if (not srt_path.exists()) or srt_path.stat().st_size == 0:
                logger.warning("[dp] SRT missing/empty; muxing without subtitles (%s)", srt_path)
                srt_path = None
        except Exception:
            srt_path = None
    ...
    try:
        ...
        logger.warning("[dp] loudnorm failed; retrying with volume filter (%s)", ex)
        _run(vol_filter)
    ...
    logger.info("[dp] mux complete → %s", out_mkv)
    return out_mkv
