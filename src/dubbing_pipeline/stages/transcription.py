...
from dubbing_pipeline.utils.net import egress_guard
...
def transcribe(
    ...
    *,
    ...
    logger.info(
        "[dp] Whisper transcribe: model=%s device=%s task=%s src_lang=%s tgt_lang=%s",
        model_name,
        device,
        task,
        src_lang,
    ...
    if job_id:
        ...
        if srt_out.exists() and meta_path.exists() and stage_is_done(ckpt, "transcribe"):
            logger.info("[dp] transcribe stage checkpoint hit")
            return srt_out
    ...
    try:
        ...
        logger.warning("[dp] Whisper not installed (%s). Writing degraded SRT/metadata.", ex)
        ...
        if hit:
            ...
            with suppress(Exception):
                ...
                if src_srt.exists() and src_meta.exists():
                    ...
                    logger.info("[dp] transcribe cache hit", key=key)
                    return srt_out
    ...
    logger.info("[dp] Wrote SRT → %s", srt_out)
    logger.info("[dp] Wrote metadata → %s", meta_path)
    return srt_out
