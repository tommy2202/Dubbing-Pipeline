...
def _detect_lang(text: str) -> str | None:
    try:
        ...
        logger.warning("[dp] langdetect unavailable/failed (%s)", ex)
        return None
...
def translate_lines(
    ...
    if src_lang.lower() == "auto":
        ...
        logger.info("[dp] translate: detected src_lang=%s", effective_src)

    if effective_src.lower() == tgt_lang.lower():
        logger.info("[dp] translate: src==tgt (%s); skipping translation", effective_src)
        return lines
    ...
    if model_override:
        logger.info("[dp] translate: using TRANSLATION_MODEL=%s", model_override)
        try:
            pipe = _try_make_pipeline(model_override, cache_dir=cache_dir)
            out_texts = [o.get("translation_text", "") for o in pipe(texts)]
        except Exception as ex:
            logger.warning(
                "[dp] translate: model override failed (%s); returning original text", ex
            )
            return lines
    else:
        # Try Marian first
        if effective_src.lower() == "auto":
            logger.warning(
                "[dp] translate: src_lang=auto but detection failed; returning original text"
            )
            return lines

        marian = f"Helsinki-NLP/opus-mt-{effective_src}-{tgt_lang}"
        try:
            logger.info("[dp] translate: trying Marian model %s", marian)
            pipe = _try_make_pipeline(marian, cache_dir=cache_dir)
            out_texts = [o.get("translation_text", "") for o in pipe(texts)]
        except Exception as ex:
            logger.warning("[dp] translate: Marian unavailable (%s). Falling back to NLLB.", ex)
            try:
                out_texts = _translate_with_nllb(
                    texts, effective_src, tgt_lang, cache_dir=cache_dir
                )
            except Exception as ex2:
                logger.warning("[dp] translate: NLLB failed (%s); returning original text", ex2)
                return lines
