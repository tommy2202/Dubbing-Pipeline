...
from dubbing_pipeline.stages.export import export_hls, export_mkv, export_mp4
from dubbing_pipeline.utils.ffmpeg_safe import ffprobe_duration_seconds, run_ffmpeg
...
def _run_demucs_if_enabled(*, audio_wav: Path, out_dir: Path, timeout_s: int) -> Path | None:
    ...
    try:
        ...
        logger.warning("[dp] demucs failed/timeout (%s); skipping separation", ex)
        return None
    ...
    if cfg.separate_vocals and cfg.enable_demucs_env:
        try:
            ...
            if bed is not None and bed.exists():
                bg_input = bed
                bg_is_wav = True
                logger.info("[dp] mixing: using demucs bed %s", bed)
            else:
                logger.info("[dp] mixing: demucs not available; using baseline ducking")
        except Exception as ex:
            logger.warning("[dp] mixing: demucs path failed (%s); using baseline ducking", ex)
    ...
    if loudnorm_target is not None:
        ...
        try:
            ...
            if meas:
                ...
                logger.info("[dp] mixing: loudnorm pass1 ok (%.2fs)", time.perf_counter() - t0)
        except Exception as ex:
            logger.warning("[dp] mixing: loudnorm pass1 failed (%s); using single-pass", ex)
            loudnorm_pass2 = (
