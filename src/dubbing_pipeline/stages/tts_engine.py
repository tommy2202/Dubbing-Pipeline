...
class CoquiXTTS(TTSEngine):
    ...
    def _load(self):
        if self._tts is not None:
            return self._tts
        logger.info(
            "[dp] Loading Coqui TTS model (via ModelManager): %s device=%s",
            self.model_name,
            self._device,
        )
    ...
    def synthesize(
        ...
        *,
        ...
        if speaker_wav is not None:
            logger.debug("[dp] XTTS clone synth (speaker_wav=%s)", speaker_wav)
            kwargs: dict[str, Any] = {
                "text": text,
                "language": language,
        ...
        logger.debug("[dp] XTTS preset synth (speaker_id=%s)", speaker_id)
        try:
            tts.tts_to_file(
                text=text,
...
def build_voice_db(*, preset_dir: Path, db_path: Path, embeddings_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning("[dp] Preset voice DB build skipped (missing deps): %s", ex)
    ...
    write_json(db_path, data)
    logger.info("[dp] Preset voice DB ready (%s presets) → %s", len(presets), db_path)
    return data
...
def choose_similar_voice(
    ...
    try:
        ...
        logger.warning("[dp] choose_similar_voice: numpy missing (%s)", ex)
        return None
    ...
    try:
        ...
        logger.warning("[dp] choose_similar_voice: failed loading target embedding (%s)", ex)
        return None
    ...
    logger.info(
        "[dp] choose_similar_voice: best=%s sim=%.3f", best_name, best_sim if best_name else -1.0
    )
    return best_name
