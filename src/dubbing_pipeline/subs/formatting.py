...
from dubbing_pipeline.utils.subtitles import write_srt, write_vtt
...
def format_subtitle_blocks_with_stats(
    ...
    for i, b in enumerate(blocks_in):
        ...
        violations = _violations_for_block(txt, max(0.0, e - s), rules)
        post_v += violations
        if violations:
...
def _load_rules_from_job_or_project(
    ...
    if project:
        try:
            ...
            pass
        try:
            from dubbing_pipeline.projects.loader import project_dir as _pdir
