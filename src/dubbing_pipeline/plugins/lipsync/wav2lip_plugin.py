...
from dubbing_pipeline.plugins.lipsync.base import LipSyncPlugin, LipSyncRequest
from dubbing_pipeline.plugins.lipsync.preview import preview_lipsync_ranges, write_preview_report
from dubbing_pipeline.utils.ffmpeg_safe import extract_audio_mono_16k, ffprobe_duration_seconds, run_ffmpeg
...
def _repo_root() -> Path:
    # src/dubbing_pipeline/plugins/lipsync/wav2lip_plugin.py -> src -> repo root
...
def _default_ckpt_candidates(repo_dir: Path) -> list[Path]:
    s = get_settings()
    return [
        # Explicit models dir path used by the legacy flow
...
class Wav2LipPlugin(LipSyncPlugin):
    ...
    def _run_wav2lip_once(
        ...
        *,
        ...
        logger.info("[dp] lipsync: cmd=%s", " ".join(cmd))
        if dry_run:
            return raw_out
    ...
    def _concat_mp4s(self, *, mp4s: list[Path], out_mp4: Path, timeout_s: int) -> Path:
        ...
        from dubbing_pipeline.utils.io import atomic_write_text
    ...
    def run(self, req: LipSyncRequest) -> Path:
        ...
        logger.info(
            "[dp] lipsync: running Wav2Lip", repo=str(paths.repo_dir), infer=str(paths.infer_py)
        )
        ...
        if bool(req.scene_limited):
            if req.ranges:
                ...
                try:
                    ...
                    logger.warning(
                        "[dp] lipsync: preview failed; falling back to pass-through (%s)", ex
                    )
                    ranges = []
        ...
        if bool(req.scene_limited) and not ranges:
            ...
            logger.warning(
                "[dp] lipsync: scene-limited enabled but no valid ranges; output will be pass-through."
            )
            if req.dry_run:
        ...
        for i, (a, b, do_ls) in enumerate(pieces):
            ...
            try:
                ...
                logger.warning(
                    "[dp] lipsync range failed; skipping",
                    start=float(a),
                    end=float(b),
                    error=str(ex),
                )
