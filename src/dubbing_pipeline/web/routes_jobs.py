...
# --- Series-scoped character voices + speaker mapping ---
...
async def post_job_speaker_mapping(
    ...
    audit_event(
        ...
        request=request,
        user_id=ident.user.id,
        meta={"job_id": str(job_id), "speaker_id": speaker_id, "character_slug": character_slug, "locked": bool(locked)},
    )
...
async def promote_series_character_ref_from_job(
    ...
    if not isinstance(man, dict):
        raise HTTPException(status_code=500, detail="invalid voice refs manifest")
    ref_path = _voice_ref_audio_path_from_manifest(base_dir=base_dir, speaker_id=speaker_id, manifest=man)
...
async def delete_series_character(
    ...
    audit_event(
        ...
        request=request,
        user_id=ident.user.id,
        meta={"series_slug": slug, "character_slug": cslug, "deleted_disk": deleted_disk, "deleted_db": deleted_db},
    )
