...
from .args_common import _assign_speakers, _parse_srt_to_cues, _select_device
from .output_format import _write_srt_from_lines, _write_vtt_from_lines
...
def run(
    ...
    if debug:
        from dubbing_pipeline.utils.log import set_log_level
    ...
    if batch_spec:
        ...
        with tempfile.TemporaryDirectory(prefix="dubbing_pipeline_batch_") as td:
            td_p = Path(td)
            ...
            if not specs:
                logger.info("[dp] batch: nothing to do")
                return

            def _run_one(spec_path: Path) -> int:
                cmd = [sys.executable, "-m", "dubbing_pipeline.batch_worker", str(spec_path)]
    ...
    try:
        from dubbing_pipeline.modes import log_effective_settings_summary, resolve_effective_settings
    ...
    series_title = str(os.environ.get("DUBBING_SERIES_TITLE") or "").strip() or str(video.stem)
    season_number = parse_int_strict(
        os.environ.get("DUBBING_SEASON_NUMBER") or os.environ.get("DUBBING_SEASON") or 1,
        "season_number",
    )
    episode_number = parse_int_strict(
        os.environ.get("DUBBING_EPISODE_NUMBER") or os.environ.get("DUBBING_EPISODE") or 1,
        "episode_number",
    )
    ...
    logger.info(
        "[dp] Starting dub: video=%s mode=%s model=%s device=%s",
        video,
        mode,
        chosen_model,
        chosen_device,
    ...
    logger.info(
        "[dp] Languages: src_lang=%s tgt_lang=%s translate=%s",
        src_lang,
        tgt_lang,
        (not no_translate),
    )
    ...
    try:
        ...
        from dubbing_pipeline.utils.job_logs import JobLogger
    ...
    try:
        from dubbing_pipeline.jobs.context import make_job_context
        from dubbing_pipeline.jobs.manifests import can_resume_stage, file_fingerprint, write_stage_manifest
    ...
    try:
        ...
        if (
            ...
            if can_resume_stage(
                ...
                extracted = wav_path
                logger.info("[dp] audio_extractor: resume hit → %s", extracted)
            else:
        ...
        logger.info(
            "[dp] audio_extractor: ok path=%s (%.2fs)", extracted, time.perf_counter() - t_stage
        )
        ...
        logger.exception("[dp] audio_extractor failed: %s", ex)
        if job_logger is not None:
            job_logger.event(stage="audio", level="error", msg="stage_failed", error=str(ex))
        logger.error("[dp] Done. Output: (failed early)")
        return
    ...
    if str(mix_mode).lower() == "enhanced":
        if sep_mode == "demucs":
            try:
                ...
                logger.warning(
                    "[dp] separation requested but unavailable; falling back to no separation (%s)",
                    ex,
                )
    ...
    try:
        ...
        if vm_enabled:
            try:
                from dubbing_pipeline.voice_memory.store import VoiceMemoryStore, compute_episode_key
        ...
        logger.info(
            "[dp] diarize: diar_segments=%s stable_speakers=%s (%.2fs)",
            len(diar_segments),
            len(set(s.get("speaker_id") for s in diar_segments)),
            time.perf_counter() - t_stage,
        )
    except Exception as ex:
        logger.exception("[dp] diarize failed (continuing): %s", ex)
    ...
    try:
        ...
        if (
            ...
            if can_resume_stage(
                ...
                logger.info("[dp] transcription: resume hit → %s", srt_out)
        if did_transcribe:
            transcribe(
        ...
        logger.info(
            "[dp] transcription: segments=%s (%.2fs) → %s",
            len(cues),
            time.perf_counter() - t_stage,
            srt_out,
        )
        ...
        logger.exception("[dp] transcription failed (continuing): %s", ex)
        if job_logger is not None:
            job_logger.event(
    ...
    if (
        ...
        try:
            ...
            logger.warning("[dp] vtt export failed (src): %s", ex)
    ...
    if subs_choice in {"src", "both"} and not priv.no_store_transcript:
        try:
            ...
            logger.warning("[dp] subs formatting failed (src): %s", ex)
    ...
    try:
        from dubbing_pipeline.review.overrides import apply_speaker_overrides_to_segments, load_overrides
    ...
    if no_translate:
        logger.info("[dp] translate: disabled (--no-translate)")
        try:
            ...
            try:
                ...
                if subs_choice in {"src", "both"} and subs_fmt in {"vtt", "both"}:
                    try:
                        ...
                        logger.warning("[dp] vtt export failed (aligned src): %s", ex)
                logger.info(
                    "[dp] align: ok (no-translate) segments=%s aligner=%s",
                    len(segments_for_mt),
                    aligner,
                )
            except Exception as ex:
                logger.warning("[dp] align: skipped/failed (no-translate) (%s)", ex)
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "[dp] align: ok segments=%s aligner=%s", len(translated_segments), aligner
                )
            except Exception as ex:
                logger.warning("[dp] align: skipped/failed (%s)", ex)
            ...
            if timing_fit:
                try:
                    ...
                    logger.warning("[dp] timing-fit skipped (%s)", ex)
            ...
            try:
                ...
                logger.warning("[dp] subs formatting failed (tgt variants): %s", ex)
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("[dp] vtt export failed (tgt): %s", ex)
            logger.info(
                "[dp] translate: segments=%s (%.2fs) → %s",
                len(translated_segments),
                time.perf_counter() - t_stage,
                translated_json,
            )
        except Exception as ex:
            logger.exception("[dp] translate failed (continuing with original text): %s", ex)
            from contextlib import suppress
    ...
    try:
        ...
        logger.info("[dp] tts: ok (%.2fs) → %s", time.perf_counter() - t_stage, tts_wav)
    except Exception as ex:
        logger.exception("[dp] tts failed (continuing with silence track): %s", ex)
    ...
    try:
        ...
        if str(mix_mode).lower() == "enhanced":
            ...
            if background_wav is not None and music_regions_path is not None:
                try:
                    from dubbing_pipeline.audio.music_detect import build_music_preserving_bed
                    from dubbing_pipeline.utils.io import read_json as _rj
        ...
        logger.exception("[dp] mix failed; falling back to simple mux: %s", ex)
        try:
            mkv_export.run(
                video=video,
            ...
            logger.info("[dp] mux fallback: ok → %s", dub_mkv)
        except Exception as ex2:
            logger.exception("[dp] mux fallback failed: %s", ex2)
            logger.error("[dp] Done. Output: (mux failed)")
            return

    logger.info("[dp] Done in %.2fs", time.perf_counter() - t0)
    if job_logger is not None:
    ...
    try:
        ...
        if mode != "off":
            ...
            if plugin is None or not plugin.is_available():
                ...
                outp = plugin.run(req)
                logger.info("[dp] lipsync: ok → %s", outp)
    except Exception as ex:
        if bool(strict_plugins) or bool(get_settings().strict_plugins):
            raise
        logger.warning("[dp] lipsync skipped (%s)", ex)

    logger.info("[dp] Done. Output: %s", dub_mkv)
