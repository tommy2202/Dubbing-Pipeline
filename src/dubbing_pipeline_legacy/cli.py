...
from dubbing_pipeline_legacy.stages import (
    audio_extractor,
    diarisation,
    downloader,
    mkv_export,
...
from dubbing_pipeline_legacy.utils import logger
...
def cli(video, src_lang, tgt_lang, mode, voice, out_dir, lipsync_flag, keep_bg_flag):
    ...
    Example: dub-legacy <video.mp4> --src-lang ja --tgt-lang en --mode high
    """
    ...
    try:
        from dubbing_pipeline_legacy.stages import translation
    ...
    if defaults["lipsync"]:
        try:
            from dubbing_pipeline_legacy.stages import lipsync
