...
def _AudioSegment():  # type: ignore
    """
    Lazy import to keep dubbing_pipeline_legacy import-safe when optional deps are missing.
    """
    try:
        ...
        raise RuntimeError(
            "dub-legacy requires `pydub`. Install it (and ensure ffmpeg is installed) "
            "or use the primary pipeline (`dubbing-pipeline`)."
        ) from ex
