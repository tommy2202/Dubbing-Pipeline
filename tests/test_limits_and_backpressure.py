...
def _job_payload(video_path: str) -> dict[str, object]:
    return {
...
def test_queue_depth_rejects_submission(tmp_path: Path) -> None:
    _set_env(tmp_path)
    os.environ["MAX_QUEUE_DEPTH_GLOBAL"] = "1"
    get_settings.cache_clear()

    video_path = Path(os.environ["INPUT_DIR"]) / "Test.mp4"
    ...
    with TestClient(app) as c:
        ...
        store.put(
            Job(
                ...
                output_srt="",
                work_dir="",
                log_path=str(Path(os.environ["DUBBING_LOG_DIR"]) / "job.log"),
            )
        )
        r = c.post("/api/jobs", json=_job_payload("Input/Test.mp4"), headers=headers)
        assert r.status_code == 429


def test_running_cap_rejects_submission(tmp_path: Path) -> None:
    _set_env(tmp_path)
    ...
    with TestClient(app) as c:
        ...
        headers = _login(c, username="user1", password="pass1")
        now = "2026-01-01T00:00:00+00:00"
...
def test_low_disk_rejects_upload_init(tmp_path: Path) -> None:
    _set_env(tmp_path)
